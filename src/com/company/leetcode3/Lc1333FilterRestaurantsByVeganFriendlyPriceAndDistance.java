...
public class Lc1333FilterRestaurantsByVeganFriendlyPriceAndDistance {
...
//        List<Pair<Integer,Integer>> set = new ArrayList<>();
        TreeSet<Pair<Integer,Integer>> set = new TreeSet<>((x,y)->
        {
        ...
        for(int [] restaurant : restaurants){
            if(veganFriendly==1){
                if(restaurant[2]==veganFriendly && restaurant[3] <= maxPrice && restaurant[4] <= maxDistance)
                    set.add(new Pair<>(restaurant[0],restaurant[1]));
            }else {
...
//        );
        ...
        for (Pair<Integer,Integer>  restaurant : set)
            integerList.add(restaurant.getKey());
        return integerList;
    }
}
