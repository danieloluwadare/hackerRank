...
In contrast, [1, 4, 7, 2, 5] and [1, 7, 4, 5, 5] are not wiggle sequences, the first because its first two differences are positive and the
 second because its last difference is zero.
A subsequence is obtained by deleting some elements (eventually, also zero) from the original sequence, leaving the remaining elements in
...
public class Lc376WiggleSubsequence {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(wiggleMaxLength(new int[]{6}));
    ...
    public static int wiggleMaxLength(int[] nums) {
        ...
        boolean bitFlip = nums[1] > nums[0];
        int count = 2;
        for (int i = 2; i < nums.length; i++){
            if(bitFlip){
                boolean flip = nums[i] > nums[i-1] && nums[i] != nums[i-1];
                if (!flip) count++;bitFlip=flip;
            }else{
                boolean flip = nums[i] > nums[i-1] && nums[i] != nums[i-1];
                if (flip) count++; bitFlip=flip;
            }
