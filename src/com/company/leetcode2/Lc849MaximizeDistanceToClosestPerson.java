...
public class Lc849MaximizeDistanceToClosestPerson {
    ...
    public static int maxDistToClosest(int[] seats) {
        ...
        int prevOneFoundIndex = 0;
        boolean foundOne = false;
        for(int i = 0; i < seats.length; i++){
            if(!foundOne && seats[i] == 1){
                maximumDistance = i - prevOneFoundIndex;
                prevOneFoundIndex=i;
                foundOne=true;
            }
            else if(foundOne && seats[i] == 1){
                int distance = i - prevOneFoundIndex;
                maximumDistance = Math.max(maximumDistance, distance / 2);
                prevOneFoundIndex=i;
            }
        }
        int distance = seats.length - 1 - prevOneFoundIndex;
